...
## 🐱 Wave 3 — UI · 커스터마이징 (진행 중)

> Level 3 (ralph 자율 루프) E1 실험 케이스로 #1 시작.

| # | 항목 | 의존성 | 상태 | 리뷰 | Level |
|---|------|--------|------|------|------|
| 1 | 고양이 색상 커스터마이즈 UI (세이브 연동) — 8 프리셋, 모달, 입력 차단, GLTF 가드 | 없음 | ✅ | [`wave3-01`](reviews/wave3-01-cat-color.md) | L3 (ralph) |
| 2 | 정보 패널 (HUD 닉네임 + 특산품 n/3) + 게이트 캡 + 토스트 + playtest 후속 튜닝 | 색상 UI | ✅ | [`wave3-02`](reviews/wave3-02-info-panel.md) | **L2** (E1 비교 데이터 확보) |
| 3 | 의상·액세서리 (리본·모자) — GLTF body 추출 시 색상도 GLTF에 적용 | 색상 UI | ⏳ | — | TBD |
| 4 | 수집 히스토리 타임라인 | 없음 | ⏳ | — | TBD |

**진행**: 2/4 완료 · 추정 잔여 1.5d

...
리뷰 이후 실기 피드백으로 추가 커밋 5건:

| 커밋 | 내용 |
|------|------|
| `b17af4b` | 토스트/모달 문구 분리 시도 + 토스트 끝에 ✨ 추가 |
| `573c819` | 두 문구에 `💡 DASH를 하면 달릴 수 있어요🐾` 섹션 추가, 토스트 wrap을 `pre-line`으로 바꿔 `\n` 실제 줄바꿈 렌더 |
| `875e837` | 토스트·모달 문구 단일 `TUTORIAL_MESSAGE`로 재병합 — 두 채널이 같은 톤/본문을 전달하는 게 더 자연스러움 |
| `3a09643` | 토스트 wrap 모드 이모지를 `display:block` + 32px로 override — 모달의 상단 큰 이모지 레이아웃과 톤 일치 |
| `4f7d5d4` | wrap 토스트 상하 패딩 10px → 18px, multi-line에서 숨통 확보 |

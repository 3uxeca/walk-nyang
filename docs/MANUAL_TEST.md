...
- [ ] 초원 마을에서 꽃 🌸 3개 수집 → 항구 마을 언락 토스트 + 진입 가능
- [ ] 항구에서 물고기 🐟 3개 → 숲 마을 언락
- [ ] 숲에서 네잎클로버 🍀 3개 → 황야 마을 언락
- [ ] 황야에서 물방울 💧 3개 수집해도 크래시 없음 (조용히 무시)
- [ ] 기본 아이템(star/coin/gem)을 아무리 모아도 지역 자동 언락 없음
- [ ] 같은 지역에서 특산품 4개째, 5개째 수집해도 토스트 반복 없음 (카운트만 증가)
- [ ] 세이브 후 새로고침 → 특산품 카운트와 언락 상태 모두 복원
...
## 고양이 색상 커스터마이즈 (Wave 3 #1)

### 진입·기본 동작
- [ ] HUD에 🎨 버튼이 표시됨 (데스크탑: 좌하단 / 모바일: 우상단 HelpButton 위)
- [ ] 🎨 버튼 클릭/탭 → 모달 오픈, 8개 프리셋 그리드 표시 (오렌지/화이트/코코아/그레이/블랙/크림/핑크/민트)
- [ ] 현재 색이 ✓ 체크 표시로 강조됨

### 닫기 트리거 (3가지 모두)
- [ ] ESC 키 → 모달 닫힘
- [ ] backdrop(카드 바깥) 클릭/탭 → 모달 닫힘
- [ ] ✕ 버튼 → 모달 닫힘

### 색상 적용 + 저장
- [ ] 프리셋 클릭/탭 → 캐릭터 메인 fur 색이 즉시 변경됨 (몸통/머리/오른쪽 귀/꼬리)
- [ ] 색상 변경 토스트 "🎨 {이름} 컬러로 변경" 표시 (2초)
- [ ] 새로고침 → 마지막 선택한 색이 유지됨
- [ ] localStorage 비우기 후 새로고침 → 기본 오렌지로 복귀, 에러 없음

### 입력 차단 (input gate)
- [ ] 모달 열려 있는 동안 WASD/방향키 입력해도 캐릭터 정지 상태 유지
- [ ] 모바일에서 모달 열려 있는 동안 가상 조이스틱·JUMP/DASH 버튼 무시됨
- [ ] 모달 닫은 직후 키/조이스틱 정상 복구

### 모바일 / 터치
- [ ] 우상단 🎨 버튼이 좌하단 VirtualJoystick과 영역 겹치지 않음
- [ ] 프리셋 swatch 터치로 선택 가능 (300ms 지연 없음)
- [ ] 모달 열림 중 화면 회전 시 레이아웃 깨지지 않음

### 키보드 a11y
- [ ] swatch에 Tab으로 포커스 이동 가능
- [ ] 포커스된 swatch에서 Enter/Space → 색상 선택

### 회귀 위험 점검
- [ ] HMR(개발 모드 코드 수정) 후에도 색상이 유지됨 — 깜빡임 없음
- [ ] 색상 변경 토스트 직후 지역 언락 토스트 발생 시 시각 충돌 없음
- [ ] GLTF cat 모델이 로드되어 있을 때 (assets에 'cat' 존재): 🎨 버튼·모달이 노출되지 않음 (no-op 회피)

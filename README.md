...
### 카메라 · 조작감
- [x] 기본 카메라를 평소보다 약간 멀리+위에서 보기로 당기고, 5초 이상 서 있으면 현재 zoom으로 서서히 전환
- [x] 대시 시 속도감 이펙트 (FOV 확대 + 먼지 파티클 트레일)
- [x] 대시 중 점프 → 일반 점프보다 더 높게 뜀
...
### UI · 피드백
- [x] 경계면(잠긴 지역/월드 끝) 닿았을 때 토스트 메시지
- [x] 랜딩(시작) 화면 — 로고 + START 버튼 (모바일/데스크탑 공통, Jua 폰트)
- [ ] 수집/언락 히스토리 타임라인
- [x] 모바일 가상 조이스틱 + JUMP/DASH 버튼
